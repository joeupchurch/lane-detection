...
lane_width_init = int(170)
upper_frame_width_init = int(59)
...
##    cv2.waitKey(0)
    ...
    if any((any(right_lines),any(left_lines))):
        ...
        cv2.line(img,(int(upper_frame_middle),int(upper_frame_row)),\
            (int(hood_middle),int(hood_row)),(0,0,255),1)

        error_text = 'P:%d I:%d D:%d' %(error[-1],\
                np.sum(integral_error),\
                (error[1]-error[0])/(error_time[1]-error_time[0]))
